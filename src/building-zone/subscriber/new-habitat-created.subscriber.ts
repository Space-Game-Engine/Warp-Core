...
export class NewHabitatCreatedSubscriber {
    ...
    async createBuildingZoneOnNewHabitatCreation(payload: HabitatCreatedEvent, transactionId: string) {
        ...
        for (let buildingZoneCounter = 0; buildingZoneCounter < counterForNewHabitat; buildingZoneCounter++) {
            await this.buildingZoneService.createNewBuildingZone(payload.habitat, transactionId);
        }
    }
}