...
import {HabitatModel, HabitatResourceModel, HabitatResourceRepository} from "@warp-core/database";
...
describe("Add resources on first user habitat", () => {
    ...
    beforeEach(async() => {
        ...
        addResourcesOnFirstHabitat = module.get(AddResourcesOnFirstHabitatSubscriber);
    ...
    describe("addResourcesToHabitat", () => {
        it("should not add any resources if they are not defined in config", async () => {
            ...
            expect(habitatResourceRepository.getSharedTransaction)
                .toBeCalledTimes(0);
            expect(habitatResourceRepository.getHabitatResourcesByIds)
                .toBeCalledTimes(0);
        });
