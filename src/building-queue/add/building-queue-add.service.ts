...
import {
    BuildingQueueElementModel, BuildingQueueRepository,
...
@Injectable()
export class BuildingQueueAddService implements ProcessAndConsumeResourcesServiceInterface{
    ...
    async processAndConsumeResources(addToQueueElement: AddToQueueInput) {
        const draftQueueElement = await this.prepareQueueElement.getQueueElement(addToQueueElement);
        ...
        try {
            const queueElement = await entityManager.save(BuildingQueueElementModel, draftQueueElement);
