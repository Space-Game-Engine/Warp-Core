...
import {BuildingQueueElementModel, BuildingQueueRepository} from "@warp-core/database";
...
import {QueueElementAfterProcessingEvent, QueueElementBeforeProcessingEvent} from "@warp-core/building-queue";
...
describe("Building queue element is saved", () => {
    ...
    describe("processAndConsumeResources", () => {
        it("should save a queue element and commit transaction", async () => {
            ...
            when(entityManager.save)
                .calledWith(
                    ...
                    draftBuildingQueueElement
                )
                .mockResolvedValue(draftBuildingQueueElement);

            const processedQueueElement = await buildingQueueAddElement.processAndConsumeResources(addToQueueElement);

            expect(processedQueueElement).toEqual(draftBuildingQueueElement);
            expect(buildingQueueRepository.commitSharedTransaction).toBeCalledTimes(1);
            expect(eventEmitter.emitAsync).toBeCalledTimes(2);
            expect(eventEmitter.emitAsync).toHaveBeenNthCalledWith(1,
                expect.stringMatching('building_queue.adding.before_processing_element'),
                expect.any(QueueElementBeforeProcessingEvent)
            );
            expect(eventEmitter.emitAsync).toHaveBeenNthCalledWith(2,
        ...
        it("should rollback shared transaction when event throws error", async() => {
            ...
            await expect(buildingQueueAddElement.processAndConsumeResources(addToQueueElement))
                .rejects
